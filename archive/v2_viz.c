 ...
 *  ------------------------------------------------------------------------
 *   data/parking_slots.txt  → occupied slot records (plate, owner, time)
 *   data/waiting_queue.txt  → vehicles in waiting queue (FIFO order)
 *   data/log_stack.txt      → entry/exit event log (push order)
 *
 *   On startup  : all three files are read and DS is fully rebuilt.
 *   On every park/remove : all three files are overwritten with
 *                          the current state (safe atomic overwrite).
 *   First run (no files) : starts fresh, files created after first op.
 *
 ...
 *   - sscanf format widths match MAX_PLATE_LEN / MAX_OWNER_LEN
 ...
 * Uses _mkdir() — Windows only (same as viz_init_dirs).
 */
void data_init_dir(void) {
    _mkdir(DATA_DIR);
}
...
void data_save_slots(void) {
    ...
    if (!fp) {
        printf("  [DATA ERROR] Cannot write %s\n", FILE_SLOTS);
        return;
    }
    int saved = 0;
    for (int i = 0; i < MAX_SLOTS; i++) {
 ...
 * data_save_queue()
 * ------------------------------------------------------------------
 * Writes every vehicle in the waiting queue (front → rear).
 *
 * Record format (one block per waiting vehicle):
 *   PLATE=DL1CA0001
 *   OWNER=Priya Singh
 *   VTYPE=1
 ...
 * On load we push them in this order so the stack is restored
 * with the correct top pointer.
 *
 * Record format (one block per log entry):
 *   EVENT=1
 *   SLOT_ID=3
 *   PLATE=MH14XY5678
 *   OWNER=Amit Kumar
...
void data_save_log(void) {
    ...
    /* Write bottom-to-top so load can just push in order */
 ...
 * data_save_all()
 * ------------------------------------------------------------------
 * Master save — call this after every state-changing operation.
 * ------------------------------------------------------------------ */
void data_save_all(void) {
    data_save_slots();
    data_save_queue();
 ...
 * data_load_all()
 * ------------------------------------------------------------------
 * Reads all three files and rebuilds DS state.
 * Called once at startup inside init_system(), AFTER all DS are
 * initialised to their empty state.
 *
 * If a file doesn't exist (first run), fopen returns NULL and we
 * simply skip that section — no error, clean fresh start.
 *
 * Load order:
 *   1. slots  → mark lot[] occupied, rebuild hashmap entries
 *   2. queue  → re-enqueue vehicles front-to-rear
 *   3. log    → re-push entries bottom-to-top
 * ------------------------------------------------------------------ */
void data_load_all(void) {
            ...
            while (fgets(line, sizeof(line), fp)) {
                ...
                if (strcmp(line, "---") == 0) {
                    ...
                    if (slot_id >= 1 && slot_id <= MAX_SLOTS &&
                        ...
                        /* Rebuild hashmap entry for this plate */
                        hashmap_insert(plate, slot_id);
                        loaded++;
                    }
                    /* Reset temporaries for next record */
...
void viz_init_dirs(void) {
    ...
    _mkdir("png");
    _mkdir("svg");
    _mkdir(DATA_DIR);   /* also ensure data/ exists for persistence */
}
 ...
 *
 * PERSISTENT STATE FILES (read on startup, written on every change):
 * ------------------------------------------------------------------------
 *  data/parking_slots.txt  → occupied slot records
 *  data/waiting_queue.txt  → waiting queue vehicles
 *  data/log_stack.txt      → entry/exit log history
 *
